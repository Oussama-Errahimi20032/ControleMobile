...
public class Quiz4 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final TextView Key4Value = findViewById(R.id.Key4Value);
        // Vérifiez si les données sont correctement récupérées depuis Firebase et attribuées à Key4Value
        databaseReference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                final String getKey4Value = snapshot.child("key_4").getValue(String.class);
                Key4Value.setText(getKey4Value);
            }

            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                // Gestion des erreurs d'accès à la base de données
                Toast.makeText(Quiz4.this, "Something went wrong", Toast.LENGTH_SHORT).show();
            }
        });
